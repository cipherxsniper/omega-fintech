...
def process_event(cur, event_id, seq, event_type, payload):
...
def main():
    ...
    # =========================
    # 🔥 CRITICAL FIX: NO DOUBLE PASSING
    # =========================
    seen = set()
    ...
    for event_id, seq, event_type, payload in events:
        ...
        process_event(cur, event_id, seq, event_type, payload)

    conn.commit()
    cur.close()
    conn.close()

    print("POSTINGS BUILT (STRICT DETERMINISTIC V9)")
