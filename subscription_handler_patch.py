...
def handle_checkout_session_completed(event, conn):
    try:
        ...
        if cur.fetchone():
            print("[SUBSCRIPTION SKIP] duplicate event:", stripe_event_id)
            return
        ...
        # ---------------------------
        cur.execute("""
            INSERT INTO stripe_event_log (event_id, event_type, payload, status, created_at)
            VALUES (?, ?, ?, ?, ?)
        """, (
            stripe_event_id,
        ...
        conn.commit()
        print("[SUBSCRIPTION OK] committed:", subscription_id)
